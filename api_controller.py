...
cities_url = 'https://data-api.oxilor.com/rest/region'
...
class CityParser:
    ...
    def get_cities(self, limit: int = 50) -> List[Dict]:
        """Получаем список городов без привязки к странам"""
        try:
            params = {'limit': limit}
            response = self.session.get(cities_url, params=params)
            response.raise_for_status()
            cities = response.json()
            print(f"Получено городов: {len(cities)}")
            return cities
        except requests.exceptions.RequestException as error:
            print(f"Ошибка при получении списка городов: {error}")
            return []
    ...
    def print_cities(self, cities):
        ...
        for i, city in enumerate(cities, 1):
            ...
            print(f"{i:3d}. {name} ({country})")

    def run(self):
        # countries = self.get_countries()
        cities = self.get_cities()
        self.print_cities(cities)
